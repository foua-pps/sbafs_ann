...
def plotScatterHisto(x_flat_in, y_flat_in, stitle, satn, xylim, pt_int, pt_str, pt_loc, histd2_bins, vmax, figname, mark_zero=False):
    ...
    xi = np.searchsorted(edgesx, x_flat)  # - edgesx[0])/(n_edges+1)).astype(np.int64)
    yi = np.searchsorted(edgesy, y_flat)  # np.floor((y - edgesy[0])/(n_edges+1)).astype(np.int64)  # -1?
    xi = xi-1
    yi = yi-1
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
#     ny_y2 = p2(xn)
    ...
    RMSE = np.sqrt(MSE)
    ax.plot(xn, xn, 'r--', lw=0.5, label = 'y=x RMSE={:3.2f} N={:d}'.format(RMSE, len(x_flat)), )
    ...
    H, xedges, yedges = np.histogram2d(x_flat, y_flat, bins=histd2_bins, range=[xylim, xylim])  # @UnusedVariable
    cmap = 'plasma'
...
def do_sbaf_plots(cfg, title_end, fig_end, what, vgac_obj_all, n19_obj_all):
    ...
    if  cfg.accept_sunz_min < 90:
        for channel in ["ch_r06", "ch_r09"]:
            ...
            figname = '{:s}/{:s}_n19_{:s}_{:s}'.format(PLOT_DIR, what, channel, fig_end)
    ...
    for channel in ["ch_tb11", "ch_tb12", "ch_tb37"]:
        ...
        plotScatterHisto(vgac_obj_all.channels[channel], n19_obj_all.channels[channel],  the_title, what, [tb_min_axis, tb_max_axis], \
                         tb_plot_ticks_int, tb_plot_ticks_str, tb_plot_ticks_loc, histd2_bins, vmax, figname)
    plt.close("all")        
    #: tb difference t11t12
    n19_t11t12_all_flat = n19_obj_all.channels["ch_tb11"] - n19_obj_all.channels["ch_tb12"]
    vgac_t11t12_all_flat = vgac_obj_all.channels["ch_tb11"] - vgac_obj_all.channels["ch_tb12"]

    
    #: tb difference t11t37
    n19_t11t37_all_flat = n19_obj_all.channels["ch_tb11"] - n19_obj_all.channels["ch_tb37"]
    vgac_t11t37_all_flat = vgac_obj_all.channels["ch_tb11"] - vgac_obj_all.channels["ch_tb37"]
    ...
    plotScatterHisto(vgac_t11t12_all_flat, n19_t11t12_all_flat, the_title, what, [tb_min_axis, tb_max_axis], \
                     tb_plot_ticks_int, tb_plot_ticks_str, tb_plot_ticks_loc, histd2_bins, vmax, figname, mark_zero=True)
    ...
    plotScatterHisto(vgac_t11t37_all_flat, n19_t11t37_all_flat, the_title, what, [tb_min_axis, tb_max_axis], \
                     tb_plot_ticks_int, tb_plot_ticks_str, tb_plot_ticks_loc, histd2_bins, vmax, figname, mark_zero=True)
...
def plot_orbit(vgac_obj, n19_obj, figname):
    ...
    ax1.plot(vgac_obj.time_dt, vgac_obj.lat[:, npp_center_scanline], label='NPP')
    ax1.plot(n19_obj.time_dt, n19_obj.lat[:, n19_center_scanline], label='Noaa 19')
...
def plot_latlon(n19_obj, viirs_obj, title_end, figname):
    ...
    if n19_obj.mask is not None:
        ...
        print("Number of data points n19 ll masked {:d}".format(len(n19_obj.lon[use])))
    else:
    ...
    if viirs_obj is not None:    
        if viirs_obj.mask is not None:
            ...
            print("Number of data points ll {:d}".format(len(viirs_obj.lon[use])))
        else:
