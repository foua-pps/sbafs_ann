...
def apply_network(Xdata, coeff_file, xscale, yscale, xmean, ymean, NUMBER_OF_TRUTHS):
    ...
    out_full = np.nan + np.empty((Xdata.shape[0], NUMBER_OF_TRUTHS, len(PERCENTILE)))
...
def train_network(Xtrain, ytrain, Xvalid, yvalid, OUTPUT_DIR= ".", NN_NAME="test"):
    ...
    model.add(Dense(N_HIDDEN_LAYER_2, kernel_initializer=INIT_DIST, activation=ACTIVATION))
    if N_HIDDEN_LAYER_3 > 0:
        model.add(Dense(N_HIDDEN_LAYER_3, kernel_initializer=INIT_DIST, activation=ACTIVATION))
        model.add(Dense(NUMBER_OF_TRUTHS * len(PERCENTILE), kernel_initializer=INIT_DIST, activation='linear'))
    model.compile(loss=lambda y, f: tilted_loss(NUMBER_OF_TRUTHS,
                                                0.01*np.array(PERCENTILE),
                                                y, f), optimizer=sgd)  # , optimizer='adagrad')
