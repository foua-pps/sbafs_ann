    ...
    def __add__(self, other):
        ...
        for channel in self.channel_list:
            if channel not in self.channels:
                continue
            self.channels[channel]  = np.ma.concatenate([self.channels[channel], other.channels[channel]])
            try:
                self.channels[channel].mask[0]
            except IndexError:
                self.channels[channel].mask = np.zeros(self.channels[channel].shape).astype(bool)
...
def getTimePerScanline(ncf):
    ...
    try:
        start_time_dt = datetime.datetime.strptime(ncf['time'].units, "days since %Y-%m-%dT%H:%M:%S.%f")
    except ValueError:
        start_time_dt = datetime.datetime.strptime(ncf['time'].units, "days since %Y-%m-%dT%H:%M:%S")
    start_time = start_time_dt - epoch
    scanLineTime = [start_time.total_seconds() + sec for sec in secFromStart]
    scanLineTime_dt = [start_time_dt + datetime.timedelta(seconds=x) for x in secFromStart]
...
def read_data(fname, cfg, exclude=[]):
    ...
    if cfg.accept_satz_max == 180:
        satza_masked = np.zeros(my['satzenith'][0,:,:].data.shape).astype(bool)
    else:
        satza_masked = my_sat['satzenith'][0,:,:].data > cfg.accept_satz_max
    if cfg.accept_sunz_max == 180:
        sunza_masked = np.zeros(my_sat['sunzenith'][0,:,:].data.shape).astype(bool)
    else:
        sunza_masked = my_sat['sunzenith'][0,:,:].data > cfg.accept_sunz_max
    if cfg.accept_sunz_min == 0:
        pass
    else:
        sunza_masked = np.logical_or(sunza_masked, my_sat['sunzenith'][0,:,:].data < cfg.accept_sunz_min)
    ...
    my_obj.time = np.tile(scanLineTime[:, np.newaxis], [1, my_obj.lat.shape[1]])
...
def mask_time_pixels(rm_obj, sat_obj, cfg):
    ...
    ok_time = np.array([time_diff[ind] < cfg.accept_time_diff * 60 for ind in range(len(time_diff))])
    if not ok_time.all():
    ...
    for channel in rm_viirs_obj.channels:
        if rm_viirs_obj.channels[channel] is not None:
            try:
                ...
                rm_viirs_obj.channels[channel].mask = np.zeros(rm_viirs_obj.channels[channel].shape).astype(bool)
...
def get_data_for_one_case(cfg, n19f, viirsf):
    ...
    xh = 2
    cutColumns(n19_obj, list(range(n19_center_scanline - xh, n19_center_scanline + xh + 1)))
...
def get_cold_37_from_viirs(viirs, n19):
    update = np.logical_and(n19.channels["ch_tb37"].mask, ~viirs.channels["ch_tb37"].mask)
...
def create_training_data(cfg, viirs, n19):
    ...
    mask = np.logical_or(n19.channels["ch_tb11"].mask, viirs.channels["ch_tb11"].mask)
    for channel in cfg.channel_list:
...
def get_matchups(cfg, n19_files, npp_files):
    ...
    for n19f in n19_files:
        ...
        for viirsf in npp_files:
            ...
            n19_time_dt = datetime.datetime.strptime(n19_start_time_s, "%Y%m%dT%H%M%S%fZ")
            npp_time_dt = datetime.datetime.strptime(npp_start_time_s, "%Y%m%dT%H%M%S%fZ")
...
def train_network_for_files(cfg, n19_files_train, n19_files_valid, npp_files):
    ...
    train_network(Xtrain, ytrain, Xvalid, yvalid, NN_NAME=nn_name, OUTPUT_DIR=cfg.output_dir)
...
def apply_network_and_plot(cfg, n19_files_test, npp_files, vgac_files):
    ...
    ytest = apply_network_nn_name(Xtest, NN_NAME=nn_name, NUMBER_OF_TRUTHS = ytest.shape[1], OUTPUT_DIR=cfg.nn_dir)
    
    vgac2_obj_all = Lvl1cObj(cfg)
    for ind, channel in enumerate(cfg.channel_list):
        if channel in n19_obj_all.channels and n19_obj_all.channels[channel] is not None:
            vgac2_obj_all.channels[channel] = 0 * vgac_obj_all.channels[channel]
    ...
    # Make same plots:
    title_end = ', SATZ < %d, SUNZ %d - %d, TD = %d min' %(cfg.accept_satz_max, cfg.accept_sunz_min, cfg.accept_sunz_max, cfg.accept_time_diff)
    fig_end = 'ch{:d}_satz-{:d}_sunz_{:d}-{:d}_td-{:d}min'.format(len(cfg.channel_list), cfg.accept_satz_max, cfg.accept_sunz_min, cfg.accept_sunz_max, cfg.accept_time_diff)  
    do_sbaf_plots(cfg, title_end, fig_end, "SBAF-VX", vgac_obj_all, n19_obj_all)
    do_sbaf_plots(cfg, title_end, fig_end, "SBAF-NN", vgac2_obj_all, n19_obj_all)
